...
class GeneralConditioner(nn.Module):
    ...
    def __init__(self, emb_models: Optional[Union[List, ListConfig]]=None):
        super().__init__()
        embedders = []
        for n, embconfig in enumerate(emb_models):
            # embedder = instantiate_from_config(embconfig)
            # embedder = IndentityEmbedder(is_trainable=True, input_key="cond_image", ucg_rate=0.0)
            embedder = IndentityEmbedder()
            embconfig= {
                'is_trainable': True,
